...
export default function StudentCourseViewPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">Loading course curriculum...</div>;
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-3">
              <div>
                ...
                <p className="text-sm text-slate-500">{completedMediaIds.size} of {mediaList.length} modules completed</p>
              </div>
                          ...
                          <Card className={`hover:border-amber-500 transition-colors cursor-pointer group-hover:shadow-md ${isCompleted ? 'bg-emerald-50/30' : ''}`}>
                            <CardContent className="p-4 flex items-center gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <h4 className="text-lg font-bold text-slate-900 truncate group-hover:text-amber-700 transition-colors">{media.title}</h4>
                                <div className="flex gap-4 text-xs text-slate-500 mt-1 uppercase tracking-wide">
                    ...
                    {legacyLessons.map(lesson => (
                      <Link key={lesson.lessonId} href={`/dashboard/student/courses/${courseId}/lessons/${lesson.lessonId}`} className="group">
                        <Card className="hover:border-amber-500 transition-colors cursor-pointer group-hover:shadow-md">
                          <CardContent className="p-4 flex items-center gap-4">
                            <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0 bg-slate-100 group-hover:bg-amber-100 group-hover:text-amber-700 text-slate-600 font-bold">
                              {lesson.order}
                            </div>
                            <div className="flex-1 min-w-0">
