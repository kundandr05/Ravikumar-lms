  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Review, reply, and manage feedback from students.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-3xl font-bold text-foreground mt-2 flex items-center gap-1">
              {stats.avgRating}
      ...
      <Card>
        <CardHeader className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between border-b bg-muted/50">
          ...
          <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
            ...
            <select 
              ...
              className="p-2 text-sm border rounded-md focus:ring-2 focus:ring-amber-500 bg-card text-card-foreground"
            >
        ...
        <CardContent className="p-0">
          {loading ? (
             <p className="text-muted-foreground text-center py-12">Loading feedback...</p>
          ) : filteredFeedbacks.length === 0 ? (
            <p className="text-muted-foreground text-center py-12">No feedback found matching the criteria.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b text-sm text-muted-foreground bg-card text-card-foreground">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredFeedbacks.map((fb) => (
                    <tr key={fb.id} className="hover:bg-muted/50 transition-colors">
                      <td className="p-4">
                        <p className="font-bold text-foreground text-sm">{fb.studentName}</p>
                        <p className="text-xs text-muted-foreground">{fb.studentEmail}</p>
                      </td>
                      <td className="p-4">
                        <p className="font-medium text-foreground text-sm">{fb.subject}</p>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card text-card-foreground rounded-xl shadow-xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            <div className="p-6 border-b flex justify-between items-center bg-muted/50 rounded-t-xl">
              <div>
                <h3 className="text-xl font-bold text-foreground">Review Feedback</h3>
                <p className="text-sm text-muted-foreground">From {selectedFeedback.studentName}</p>
              </div>
              <button onClick={() => setSelectedFeedback(null)} className="text-muted-foreground hover:text-muted-foreground">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="space-y-2">
                <div className="flex gap-2 mb-2">
                  <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{selectedFeedback.category}</span>
                </div>
                <h4 className="font-bold text-lg text-foreground">{selectedFeedback.subject}</h4>
                <div className="p-4 bg-muted/50 rounded-lg text-foreground text-sm whitespace-pre-wrap border">
              ...
              <form onSubmit={handleUpdate} className="space-y-4 pt-4 border-t border-slate-100">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-foreground">Admin Reply</label>
                  <textarea 
                    value={replyText}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">This reply will be visible to the student in their dashboard.</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full p-3 border rounded-md focus:ring-2 focus:ring-amber-500 bg-card text-card-foreground text-sm"
                  >
                    <option value="pending">Pending</option>
